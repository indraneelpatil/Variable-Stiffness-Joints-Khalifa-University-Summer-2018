...
{
ros::init(argc, argv, "EPOS_motor_angle");
ros::NodeHandle nodehandle;
ros::Publisher pub=nodehandle.advertise<std_msgs::Float32>("EPOS_motor_angle",5);
}
...
Epos::Epos(const std::string& name,
  ...
  : name_(name), config_nh_(config_nh), diagnostic_updater_(nh, config_nh), epos_factory_(epos_factory),
    ...
    position_cmd_(0), velocity_cmd_(0), torque_cmd_(0) {
...
//!VCS_GetDeviceErrorCode(node_handle_->device_handle->ptr, node_handle_->node_id, 1, &error_number, &error_code);
//ROS_WARN_STREAM("EPOS Device Error: 0x" << std::hex << error_number);
...
// VCS_GetCurrentIs(node_handle_->device_handle->ptr, node_handle_->node_id, &current_raw, &error_code);
 ...
 //current_ = current_raw; // mA -> A
  //effort_ = currentToTorque(current_);

/*extern ros::Publisher pub;

std_msgs::Float32 motor_angle;
motor_angle.data=float(position_);
 ...
 //   return;
  ...
  if(operation_mode_ == PROFILE_VELOCITY_MODE) {
    ...
    if(isnan(torque_cmd_))
      return;
    //std::cout<<"\n Set Current is:"<<(int)torqueToCurrent(torque_cmd_*100)   ;
    VCS_SetCurrentMust(node_handle_->device_handle->ptr, node_handle_->node_id, (int)torqueToCurrent(torque_cmd_*100), &error_code);
  }
}
...
void Epos::buildMotorOutputStatus(diagnostic_updater::DiagnosticStatusWrapper &stat) {
  ...
  if(has_init_) {
    ...
    stat.add("Current", boost::lexical_cast<std::string>(current_) + " mA");
